...
def query_messages(senders, subjects):
    """Query cached messages by sender and/or subject."""
    conn = get_connection()
    cur = conn.cursor()
    where = []
    params = []
    ...
    cur.execute(f"SELECT * FROM messages WHERE {clause} AND is_deleted=0", params)
    rows = cur.fetchall()
    conn.close()
    return rows
...
def main():
    parser = argparse.ArgumentParser(description="Query and optionally delete Gmail messages from Gmail and local DB.")
    ...
    parser.add_argument("--delete", action="store_true", help="Delete matching messages from Gmail and mark deleted in DB.")
    parser.add_argument("--account", help="Specify Gmail account to authenticate (login_hint).")
    args = parser.parse_args()

    rows = query_messages(args.senders, args.subject)
    print(f"Found {len(rows)} messages matching filters.\n")
    ...
    if args.delete:
        ...
        cur = conn.cursor()
        for r in rows:
            cur.execute("UPDATE messages SET is_deleted=1 WHERE rfc822_id=?", (r["rfc822_id"],))
        conn.commit()
        conn.close()
        print(f"✅ Updated {len(rows)} records in database as deleted.")
    else:
        print("\nRun again with --delete to remove these messages from Gmail and mark them deleted.")
